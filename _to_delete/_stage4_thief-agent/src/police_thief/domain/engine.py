...
Turn order is **thief first, then cop** - a free choice, since the rulebook does
not fix one. This module does not implement it: the engine only applies actions,
and the ordering lives in the one networked turn loop
(``scripts/_series_lib.play_networked``), which sends the thief's move before
waiting. This paragraph claimed "cop first" until 2026-08-20 and was wrong, as
was ``shared.interop_profile.TURN_ORDER``, which is what we DECLARED to
opponents while playing the opposite.

It is not a *local* choice: two peers on opposite orders shake hands, play, and
then disagree about the board, producing hash-clean logs with divergent
histories. It is therefore declared at the handshake where a difference refuses.
"""
