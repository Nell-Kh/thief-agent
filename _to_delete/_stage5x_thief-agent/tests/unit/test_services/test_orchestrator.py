...
def test_the_orchestrator_owns_every_subsystem(
    police: Orchestrator, config_dir: Path
) -> None:
    ...
    # From the CONTRACT, not a literal: the watchdog threshold is negotiable
    # (PRD_p2p_mcp), and what this test is about is that the orchestrator wires
    # it, not what the two teams happened to agree. Loaded the same way the
    # fixture does rather than read off the orchestrator's private config -
    # this is the test that asserts nothing reaches through another module.
    contract = ConfigManager.load("police", config_dir).contract
    assert police.watchdog.timeout_sec == contract.network.watchdog_timeout_sec
