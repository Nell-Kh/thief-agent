...
def test_a_late_reply_is_treated_as_a_failure(build, fake_clock, network_config) -> None:
    """A missed deadline is a failure, not an invitation to keep waiting."""
    clock = fake_clock
    # One second past whatever deadline the contract carries. Hardcoding 31
    # silently stopped testing anything the moment the two teams negotiated a
    # longer response timeout - the transport was late by the old number and
    # early by the new one, so the deadline never fired.
    overrun = network_config.response_timeout_sec + 1
