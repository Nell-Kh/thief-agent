...
| 8.19 | **Repo split + tag** — published, tagged and verified; only the Moodle paste (8.19.3) remains, at submission time | P0 | team | ✔ |
| 8.19.1 | DONE: `Nell-Kh/police-agent` and `Nell-Kh/thief-agent` published from the split tool's trees, `main` pushed on both | P0 | team | ✔ |
| 8.19.2 | Both repos carry the whole tracked `config/` — BOTH roles' TOMLs, not a per-role subset: rule #50 wants `config/` in each repo and the mutual audit needs each side able to re-derive the other's terms — plus the whole `docs/` set. The split tool tailors exactly one thing, the README role banner, whose cross-links were verified in both built trees | P0 | team | ✔ |
| 8.19.3 | Four links in the result JSON ✔ (read from `[game].repos`); the two Moodle links are pasted at submission time (`docs/SUBMISSION.md` §3) | P0 | team | ◐ |
| 8.19.4 | DONE: annotated `v1.0-submission` pushed in both (verified via `git ls-remote`: tag object + deref commit present); repos publicly reachable without auth | P0 | team | ✔ |
| 8.19.5 | Partition decided and documented: full shared engine per repo (the mutual audit forces each side to carry the other's physics; rule 50 wants the full doc set anyway), role identity in the README banner | P0 | team | ✔ |
| 8.19.6 | Scrub by construction: trees are copied from `git ls-files`, so untracked scratch, caches and secrets cannot travel — verified no credentials/token/.env in either build | P0 | team | ✔ |
| 8.19.7 | police-agent README: role banner + cross-link + full report, written by the split tool | P0 | team | ✔ |
| 8.19.8 | thief-agent README: role banner + cross-link + full report, written by the split tool | P0 | team | ✔ |
| 8.19.9 | Verified: full suite + ruff run INSIDE each built tree — 775 passed, ruff clean, in both | P0 | team | ✔ |
| 8.19.10 | DONE: both repos publicly reachable (verified logged-out); second team member added as collaborator | P0 | team | ✔ |
| 8.19.11 | Both URLs in `[game].repos` of both TOMLs (the result's four-link block reads from there) and in README §11 | P0 | team | ✔ |
...
| # | Task | Priority | Owner | Status |
|---|---|---|---|---|
| 11.1 | **Submission blockers — the six gates that fail independently of code quality.** All six are cleared: both role repos published and grafted onto the real history, annotated tags pushed, README §11 and both TOMLs carrying the two real URLs, the four lifecycle artifacts tracked, the tree clean. 11.1.9 stays ◐ *by design* — it re-opens once per counted game | P0 | team | ✔ |
...
| 11.1.1 | DONE: both repos published and publicly reachable, annotated tags pushed | P0 | team | ✔ |
| 11.1.2 | DONE: each published README opens with the role banner linking the counterpart repo (live at the real URLs) | P0 | team | ✔ |
| 11.1.3 | Fixed: both TOMLs now name `Nell-Kh/police-agent` and `Nell-Kh/thief-agent`; the split tool refuses to build while the table lies (duplicate or non-GitHub URLs) | P0 | team | ✔ |
| 11.1.4 | DONE: `README.md` §11 carries the two real URLs (`Nell-Kh/police-agent`, `Nell-Kh/thief-agent`); the `TBD — task 8.19` placeholder is gone, and the same pair feeds the result JSON's four-link block from `[game].repos` | P0 | team | ✔ |
| 11.1.5 | DONE: both repos carry `README`, `config/`, all `PRD_*`, `PLAN.md` and `TODO.md` (rule #50) — verified by blob-hash diff against the dev tree, which is identical in every tracked path except `README.md` | P0 | team | ✔ |
...
| 11.1.8 | Un-ignore the four lifecycle artifacts: `.gitignore` narrowed to secrets, caches and scratch (`*.tmp`, `rows_checkpoint.json`, `*.superseded-*`; `instruction/*.pdf` joined them at 11.5.11, as third-party course material rather than our output) — App. ו Mandatory Rules #4 requires every game's config file in the repo | P0 | team | ✔ |
...
| 11.1.12 | DONE: annotated `v1.0-submission` created with `-a -m` and pushed on BOTH role repos (rule #41, App. ג checklist). It marks the submission commit on each role repo, never the dev repo — tagging the pre-split dev repo would have pointed it at the wrong tree. **Re-run the re-tag block in `docs/SUBMISSION.md` §1 after any re-graft**, so the tag never lags the tip | P0 | team | ✔ |
| 11.1.13 | DONE: `git show v1.0-submission` on both repos reports a tag object carrying its message (not a bare commit), dereferencing to that repo's `main` tip — the commit that actually played | P1 | team | ✔ |
| 11.1.14 | DONE: `main` pushed on all three repos; each published HEAD matches the local one. The dev repo is the provenance anchor and is never force-pushed | P0 | team | ✔ |
...
| 11.3.6 | Purged from every artifact that can honestly be rewritten: `scripts/_scratch_config_sparring/` regenerated from the real `config/`, so it now carries `yanell11` / `YANELL11` and the two real repo URLs (it is a copy of `config/` made by `_sparring_subgame.make_scratch_config`, and the committed copy predated the identity fix). ONE placeholder deliberately stays: `logs/log_replay-demo_g01.json` carries `group_name: "TEAM-TBD"` inside a **sealed commit-reveal payload**, so editing the bytes would break the very `Verified OK` the rule-9.4.2 replay screenshot proves. It is a local demo log (`github_commit: "localdemo"`), never a league artifact, and is regenerated wholesale by `scripts/capture_replay_viewer.py` | P1 | team | ◐ |
...
| 11.5.10 | `docs/REVIEW_HOSTILE.md` date-stamped and given a disposition table. It is a **point-in-time** adversarial review (699 tests / 97.8 %) and its blockers B1, B2 and M6 read as current conditions long after they were fixed — a reader could not tell which findings still stood. The findings themselves are left verbatim, because a review rewritten after the fact stops being evidence that the review happened | P1 | team | ✔ |
| 11.5.11 | Stopped republishing the lecturer's own PDFs. `instruction/*.pdf` (the assignment and the submission guidelines, 1.7 MB) were tracked and therefore travelled into two PUBLIC role repos. Untracked via `git rm --cached` and added to `.gitignore`; they stay on the authors' disks, and the four citations in `docs/COMPLIANCE.md` and `docs/REVIEW_HOSTILE.md` now name them as course-issued documents rather than linking a path a reader cannot open | P1 | team | ✔ |
